...
def calculate_sum_of_large_frames_inProjecrID(json_data, threshold):
    ...
    print(global_repetition_counts_sum)

'''


def count_rows_above_threshold(json_data, threshold):
    count_values = get_values(json_data.get("results", {}), "count")
    filtered_values = [count for count in count_values if count >= threshold]
...
def calculate_sum_of_large_frames_inProjecrID(json_data, threshold):
    ...
    for index, payload in enumerate(payload_values):
        try:
            payload_dict = json.loads(payload)
            shapes = payload_dict.get("shapes", [])
            for shape in shapes:
...
def find_rows_with_two_different_frames(json_data):
    payload_values = get_values(json_data.get("results", {}), "payload")
    indices = []
...
def print_frames_and_count(json_data):
    payload_values = get_values(json_data.get("results", {}), "payload")
    count_values = get_values(json_data.get("results", {}), "count")
    ...
    for index, payload in enumerate(payload_values):
        try:
            ...
            if len(frame_count) == 2:
                frame_values = list(frame_count.keys())
                frame_repetitions = list(frame_count.values())
                print(f"Distinct frame values: {frame_values}, Repetitions: {frame_repetitions}")
                count_value = count_values[index]
                print(f"Corresponding count value at index {index}: {count_value}")
        except json.JSONDecodeError:
            print("Error decoding payload:", payload)
...
def get_start_and_end_dates(json_data):
...
def get_unique_usernames(json_data):
...
if __name__ == "__main__":
    ...
    if json_data:
        ...
        get_start_and_end_dates(json_data)
        get_unique_usernames(json_data)
        insert_values_into_database()



        row_count_above_threshold = count_rows_above_threshold(json_data, threshold)
        print("Number of rows with count greater than or equal to threshold:", row_count_above_threshold)

        indices = find_rows_with_two_different_frames(json_data)
        print("Indices of rows with two different frames:", indices)
        
        print_frames_and_count(json_data)